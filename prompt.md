...
5. ユーザーの指示で続行
6. スライドの内容をアーティファクトを使用してMarkdownの下書きとして作成し、ユーザーに確認を求めてください
7. ユーザーの指示で続行
8. Reactコンポーネントとして完全なコードを出力してください
9. Tailwind CSSを使用し、視覚的に魅力的なデザインを実現してください
10. 最大文字数に達して中断された場合、「続ける」の指示で続きから書いてください（最初から書き直さないこと）
11. Reactコード生成後にユーザーからエラーの指摘があった場合、まずテキストエレメントの不等号`<`,`>`とアンパサンド`&`が文字実体参照になっているかを確認してください
